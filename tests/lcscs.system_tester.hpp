...
public:
   ...
   void basic_setup() {
      ...
      create_accounts({ N(lcscs.token), N(lcscs.ram), N(lcscs.ramfee), N(lcscs.stake),
               N(lcscs.bpay), N(lcscs.vpay), N(lcscs.saving), N(lcscs.names), N(lcscs.rex) });
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(lcscs.token), config::system_account_name, asset(100000000000000, core_symbol) );
      issue( asset(10000000000000, core_symbol) );
      BOOST_REQUIRE_EQUAL( asset(10000000000000, core_symbol), get_balance( "lcscs", core_symbol ) );
   }
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("lcscs")  + get_balance("lcscs.ramfee") + get_balance("lcscs.stake") + get_balance("lcscs.ram") );
   }
   ...
   lcscs_system_tester( setup_level l = setup_level::full ) {
   ...
   action_result buyram( std::string_view payer, std::string_view receiver, const asset& scsin ) {
      return buyram( account_name(payer), account_name(receiver), scsin );
   }
   ...
   fc::variant get_last_loan(bool cpu) {
      ...
      namespace chain = lcscs::chain;
      auto table = cpu ? N(cpuloan) : N(netloan);
      const auto* t_id = db.find<lcscs::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, table ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_pool() const {
      ...
      const auto& db = control->db();
      namespace chain = lcscs::chain;
      const auto* t_id = db.find<lcscs::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(rexpool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_return_pool() const {
      ...
      const auto& db = control->db();
      namespace chain = lcscs::chain;
      const auto* t_id = db.find<lcscs::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(rexretpool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_return_buckets() const {
      ...
      const auto& db = control->db();
      namespace chain = lcscs::chain;
      const auto* t_id = db.find<lcscs::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(retbuckets) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(lcscs.token), act, N(accounts), account_name(balance_symbol.to_symbol_code().value) );
   ...
   void issue_and_transfer( const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      signed_transaction trx;
      trx.actions.emplace_back( get_action( N(lcscs.token), N(issue),
      ...
      if ( to != manager ) {
         trx.actions.emplace_back( get_action( N(lcscs.token), N(transfer),
                                               vector<permission_level>{{manager, config::active_name}},
                                               mutable_variant_object()
                                               ("from",     manager)
                                               ("to",       to )
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( N(lcscs.token), name(symbol_code), N(stat), account_name(symbol_code) );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "lcscs_global_state", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state2() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "lcscs_global_state2", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state3() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "lcscs_global_state3", data, abi_serializer_max_time );
   }
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( N(lcscs), N(lcscs.msig), core_sym::from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( N(lcscs), N(alice1111111), core_sym::from_string("650000000.0000"), config::system_account_name );
   ...
   action_result setinflation( int64_t annual_rate, int64_t inflation_pay_factor, int64_t votepay_factor ) {
      return push_action( N(lcscs), N(setinflation), mvo()
...
inline uint64_t M( const string& scs_str ) {
   return core_sym::from_string( scs_str ).get_amount();
}
