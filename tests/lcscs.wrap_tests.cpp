...
class lcscs_wrap_tester : public tester {
public:

   lcscs_wrap_tester() {
      create_accounts( { N(lcscs.msig), N(prod1), N(prod2), N(prod3), N(prod4), N(prod5), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(lcscs.wrap), contracts::wrap_wasm(), &system_private_key );
      set_abi( N(lcscs.wrap), contracts::wrap_abi().data(), &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(lcscs.wrap) );
      abi_def abi;
      BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
      abi_ser.set_abi(abi, abi_serializer_max_time);

      while( control->pending_block_producer().to_string() == "lcscs" ) {
         produce_block();
      }
   }

   void propose( name proposer, name proposal_name, vector<permission_level> requested_permissions, const transaction& trx ) {
      push_action( N(lcscs.msig), N(propose), proposer, mvo()
                     ("proposer",      proposer)
   ...
   void approve( name proposer, name proposal_name, name approver ) {
      push_action( N(lcscs.msig), N(approve), approver, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
   ...
   void unapprove( name proposer, name proposal_name, name unapprover ) {
      push_action( N(lcscs.msig), N(unapprove), unapprover, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
...
transaction lcscs_wrap_tester::wrap_exec( account_name executer, const transaction& trx, uint32_t expiration ) {
   fc::variants v;
...
transaction lcscs_wrap_tester::reqauth( account_name from, const vector<permission_level>& auths, uint32_t expiration ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(lcscs_wrap_tests)

BOOST_FIXTURE_TEST_CASE( wrap_exec_direct, lcscs_wrap_tester ) try {
...

   {
      ...
      wrap_trx.actions.emplace_back( get_action( N(lcscs.wrap), N(exec),
                                                 {{N(alice), config::active_name}, {N(lcscs.wrap), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, lcscs_wrap_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   ...
   push_action( N(lcscs.msig), N(exec), N(alice), mvo()
                  ("proposer",      "carol")
                  ("proposal_name", "first")
                  ("executer",      "alice")
   );
   ...
   BOOST_REQUIRE_EQUAL( N(lcscs.wrap), name{traces[0]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_unapprove, lcscs_wrap_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   auto wrap_trx = wrap_exec( N(alice), trx );
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of lcscs.wrap::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(lcscs.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_producers_change, lcscs_wrap_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of lcscs.wrap::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(lcscs.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
   ...
   BOOST_REQUIRE_EXCEPTION( approve( N(carol), N(first), N(newprod1) ),
                            lcscs_assert_message_exception,
                            lcscs_assert_message_is("approval is not on the list of requested approvals")
   );
