...
class lcscs_msig_tester : public tester {
public:
   lcscs_msig_tester() {
      create_accounts( { N(lcscs.msig), N(lcscs.stake), N(lcscs.ram), N(lcscs.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(lcscs.msig), contracts::msig_wasm() );
      set_abi( N(lcscs.msig), contracts::msig_abi().data() );
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(lcscs), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(lcscs), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(lcscs.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(lcscs.msig), name, accounts, data );
      produce_block();
...
transaction lcscs_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(lcscs_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, lcscs_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            lcscs_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, lcscs_msig_tester ) try {
   auto trx = reqauth( N(alice), {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, lcscs_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, lcscs_msig_tester ) try {
   auto trx = reqauth( N(alice), vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, lcscs_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, lcscs_msig_tester ) try {

   // required to set up the link between (lcscs active) and (lcscs.prods active)
   //
   //                  lcscs active
   //                       |
   //             lcscs.prods active (2/3 threshold)
   ...
   set_authority(
      ...
      authority( 1,
                 ...
                 vector<permission_level_weight>{{{N(lcscs.prods), config::active_name}, 1}}
      ),
      config::owner_name,
   ...
   create_accounts( { N(lcscs.token), N(lcscs.rex) } );
   set_code( N(lcscs.token), contracts::token_wasm() );
   set_abi( N(lcscs.token), contracts::token_abi().data() );

   create_currency( N(lcscs.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance(config::system_account_name) + get_balance(N(lcscs.ramfee)) + get_balance(N(lcscs.stake)) + get_balance(N(lcscs.ram)) );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(lcscs), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(lcscs), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(lcscs), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(lcscs), config::active_name}};
   ...
   // execute by alice to replace the lcscs system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect(
   [&]( std::tuple<const transaction_trace_ptr&, const signed_transaction&> p ) {
      const auto& t = std::get<0>(p);
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(lcscs), core_sym::from_string("1.0000"), false ),
                            lcscs_assert_message_exception, lcscs_assert_message_is("rejecting all actions")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, lcscs_msig_tester ) try {

   // set up the link between (lcscs active) and (lcscs.prods active)
   set_authority(
      config::system_account_name,
      config::active_name,
      authority( 1,
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      lcscs_assert_message_exception, lcscs_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_invalidate, lcscs_msig_tester ) try {
   auto trx = reqauth( N(alice), {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_invalidate_approve, lcscs_msig_tester ) try {
   auto trx = reqauth( N(alice), {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, lcscs_msig_tester ) try {
   set_code( N(lcscs.msig), contracts::util::msig_wasm_old() );
   set_abi( N(lcscs.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of lcscs.msig
   auto trx = reqauth( N(alice), {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
...
BOOST_FIXTURE_TEST_CASE( approve_unapprove_old, lcscs_msig_tester ) try {
   set_code( N(lcscs.msig), contracts::util::msig_wasm_old() );
   set_abi( N(lcscs.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_by_two_old, lcscs_msig_tester ) try {
   set_code( N(lcscs.msig), contracts::util::msig_wasm_old() );
   set_abi( N(lcscs.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_with_hash, lcscs_msig_tester ) try {
   auto trx = reqauth( N(alice), {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(approve), mvo()
                            ...
                            lcscs::chain::crypto_api_exception,
...
BOOST_FIXTURE_TEST_CASE( switch_proposal_and_fail_approve_with_hash, lcscs_msig_tester ) try {
