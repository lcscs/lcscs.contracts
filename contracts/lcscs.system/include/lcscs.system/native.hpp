...
#include <lcscs/action.hpp>
#include <lcscs/contract.hpp>
#include <lcscs/crypto.hpp>
#include <lcscs/fixed_bytes.hpp>
#include <lcscs/ignore.hpp>
#include <lcscs/print.hpp>
...
namespace lcscssystem {
   ...
   using lcscs::public_key;

   /**
    * @addtogroup lcscssystem
    * @{
    */
   /**
   ...
   struct permission_level_weight {
      ...
      scsLIB_SERIALIZE( permission_level_weight, (permission)(weight) )
   };
    ...
    */
   struct key_weight {
      lcscs::public_key  key;
      uint16_t           weight;

      // explicit serialization macro is not necessary, used here only to improve compilation time
      scsLIB_SERIALIZE( key_weight, (key)(weight) )
   };
   ...
   struct wait_weight {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      scsLIB_SERIALIZE( wait_weight, (wait_sec)(weight) )
   };
   ...
   struct authority {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      scsLIB_SERIALIZE( authority, (threshold)(keys)(accounts)(waits) )
   };
   ...
   struct block_header {
      ...
      std::optional<lcscs::producer_schedule>   new_producers;

      // explicit serialization macro is not necessary, used here only to improve compilation time
      scsLIB_SERIALIZE(block_header, (timestamp)(producer)(confirmed)(previous)(transaction_mroot)(action_mroot)
    ...
    */
   struct [[lcscs::table("abihash"), lcscs::contract("lcscs.system")]] abi_hash {
      name              owner;
      checksum256       hash;
      uint64_t primary_key()const { return owner.value; }

      scsLIB_SERIALIZE( abi_hash, (owner)(hash) )
   };

   // Method parameters commented out to prevent generation of code that parses input data.
   /**
    * The lcscs core `native` contract that governs authorization and contracts' abi.
    */
   class [[lcscs::contract("lcscs.system")]] native : public lcscs::contract {
      public:
         ...
         /**
          * @{
          * These actions map one-on-one with the ones defined in core layer of lcscs, that's where their implementation
          * actually is done.
          ...
          * This is useful because when doing authorization checks, the lcscs based blockchain starts with the
          * action needed to be authorized (and the contract belonging to), and looks up which permission
          * is needed to pass authorization validation. If a link is set, that permission is used for authoraization
          * validation otherwise then active is the default, with the exception of `lcscs.any`.
          * `lcscs.any` is an implicit permission which exists on every account; you can link actions to `lcscs.any`
         ...
         using newaccount_action = lcscs::action_wrapper<"newaccount"_n, &native::newaccount>;
         using updateauth_action = lcscs::action_wrapper<"updateauth"_n, &native::updateauth>;
         using deleteauth_action = lcscs::action_wrapper<"deleteauth"_n, &native::deleteauth>;
         using linkauth_action = lcscs::action_wrapper<"linkauth"_n, &native::linkauth>;
         using unlinkauth_action = lcscs::action_wrapper<"unlinkauth"_n, &native::unlinkauth>;
         using canceldelay_action = lcscs::action_wrapper<"canceldelay"_n, &native::canceldelay>;
         using setcode_action = lcscs::action_wrapper<"setcode"_n, &native::setcode>;
         using setabi_action = lcscs::action_wrapper<"setabi"_n, &native::setabi>;
   };
   /** @}*/ // @addtogroup lcscssystem
}
