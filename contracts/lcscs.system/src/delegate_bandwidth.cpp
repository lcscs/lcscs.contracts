...
// Unfortunately, this is needed until CDT fixes the duplicate symbol error with lcscs::send_deferred
...
namespace lcscssystem {
   ...
   void system_contract::buyrambytes( const name& payer, const name& receiver, uint32_t bytes ) {
      ...
      const int64_t scs_reserve   = itr->quote.balance.amount;
      const int64_t cost          = exchange_state::get_bancor_input( ram_reserve, scs_reserve, bytes );
  ...
  /**
                                   ...
                                   const asset& stake_net_delta, const asset& stake_cpu_delta, bool transfer )
   {
      ...
      if ( stake_account != source_stake_from ) { //for lcscs both transfer and refund make no sense
         ...
         if ( need_deferred_trx ) {
            lcscs::transaction out;
            out.actions.emplace_back( permission_level{from, active_permission},
                                      get_self(), "refund"_n,
                                      from
            );
            out.delay_sec = refund_delay_sec;
            lcscs::cancel_deferred( from.value ); // TODO: Remove this line when replacing deferred trxs is fixed
            out.send( from.value, from, true );
         } else {
            lcscs::cancel_deferred( from.value );
         }
