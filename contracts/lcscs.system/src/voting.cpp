...
#include <lcscs/multi_index.hpp>
#include <lcscs/permission.hpp>
#include <lcscs/privileged.hpp>
#include <lcscs/serialize.hpp>
#include <lcscs/singleton.hpp>

#include <lcscs.system/lcscs.system.hpp>
#include <lcscs.token/lcscs.token.hpp>
...
namespace lcscssystem {
   ...
   using lcscs::current_time_point;
   using lcscs::indexed_by;
   using lcscs::microseconds;
   using lcscs::singleton;

   void system_contract::register_producer( const name& producer, const lcscs::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      auto prod = _producers.find( producer.value );
      const auto ct = current_time_point();

      lcscs::public_key producer_key{};
   ...
   void system_contract::regproducer( const name& producer, const lcscs::public_key& producer_key, const std::string& url, uint16_t location ) {
   ...
   void system_contract::regproducer2( const name& producer, const lcscs::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      require_auth( producer );
      check( url.size() < 512, "url too long" );
   ...
   void system_contract::update_elected_producers( const block_timestamp& block_time ) {
      ...
      using value_type = std::pair<lcscs::producer_authority, uint16_t>;
      std::vector< value_type > top_producers;
      top_producers.reserve(21);

      for( auto it = idx.cbegin(); it != idx.cend() && top_producers.size() < 21 && 0 < it->total_votes && it->active(); ++it ) {
         top_producers.emplace_back(
            lcscs::producer_authority{
      ...
      std::vector<lcscs::producer_authority> producers;
